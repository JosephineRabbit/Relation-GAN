...
class RE_Critic(nn.Module):
    def __init__(self, nhidden, nlayers):
        super(RE_Critic, self).__init__()
        # self.net = nn.Sequential(
        #     nn.Linear(2, nhidden),
        #     nn.ReLU(True),
        #     nn.Linear(nhidden, nhidden),
        ...
        self.re.add_module('output', nn.Linear(nhidden, 1))

    def forward(self, x1,x2):
        return self.re(self.net(x1)+self.net(x2)).view(-1)
...
def visualize_grad_re(G: Generator, D: Discriminator, criterion, fig, ax, scale, device=None):
    nticks = 20
    noise_batch = (torch.rand(nticks * nticks, 2, device=device) - 0.5) * 4
    ones = torch.ones(nticks * nticks, 1, device=device)
    ...
    noise_batch.requires_grad_()
    with torch.enable_grad():
        out_batch = D(noise_batch,noise_batch)
...
def REGAN(D, G, data, noise, niter=10000, ncritic=5, batch_size=32, optimizer='Adam',
                lrg=1e-3, lrd=3e-3, center=0, LAMBDA=1, alpha=None, device='cuda', prefix='figs/', args=None):
    ...
    for iter in range(niter):
        if iter % 1000 == 0:
            ...
            visualize_grad_re(G, D, criterion, fig, ax, scale=scale, device=device)
            plt.draw()
            plt.savefig(prefix + 'fig_%05d.pdf' % iter, bbox_inches='tight')
            plt.pause(0.1)
        ...
        for i in range(ncritic):
            ...
            real_batch_1 = real_batch.to(device)

            real_batch_2 = data.next_batch(batch_size)
            real_batch_2 = real_batch_2.to(device)

            D_rr = D(real_batch_1,real_batch_2).mean()

            noise_batch = noise.next_batch(batch_size)
            noise_batch = noise_batch.to(device)
            fake_batch_1 = G(noise_batch)
            fake_batch_1 = fake_batch_1.detach()
            ...
            if flag==False:
               ...
               # fake_batch_2 = G(noise_batch_2)
                fake_batch_2 = fake_batch_l.detach()
            ...
            D_ff = D(fake_batch_1,fake_batch_2)
            D_fr = D(fake_batch_1, real_batch_1)
            ...
            loss_d = nn.ReLU()(D_ff-D_fr+100).mean()+nn.ReLU()(D_rr-D_fr+100).mean()
            loss_d.backward()
            optim_d.step()
        ...
        real_batch_1 = real_batch.to(device)
        noise_batch = noise.next_batch(batch_size)
        noise_batch_1 = noise_batch.to(device)
        noise_batch_2 = noise.next_batch(batch_size)
        noise_batch_2 = noise_batch_2.to(device)
        if flag == False:
            fake_batch_2 = G(noise_batch_2)
        ...
        flag = True
        fake_batch_1 = G(noise_batch_1)
        fake_batch_2 = G(noise_batch_2)

        loss_g = 2*D(fake_batch_1,real_batch_1).mean()-D(fake_batch_1,fake_batch_2).mean()

        loss_g.backward()
        fake_batch_l = fake_batch_1.detach()
        optim_g.step()
...
def RelativGAN(D, G, data, noise, niter=10000, ncritic=5, batch_size=32, optimizer='Adam',
                lrg=1e-3, lrd=3e-3, center=0, LAMBDA=1, alpha=None, device='cuda', prefix='figs/', args=None):
    D.apply(weights_init)
    G.apply(weights_init)
    D.to(device)
    ...
    if optimizer == 'SGD':
        ...
        optim_d = optim.RMSprop(D.parameters(),lr=lrd)
        optim_g = optim.RMSprop(G.parameters(), lr=lrg)
    ...
    for iter in range(niter):
        ...
        for i in range(ncritic):
            ...
            fake_batch = G(noise_batch)
            fake_batch = fake_batch.detach()
            y_pred_fake = D(fake_batch)
            loss_d=(torch.mean(torch.nn.ReLU()(1.0 - (y_pred - torch.mean(y_pred_fake)))) + torch.mean(
        ...
        fake_batch = G(noise_batch)
        y_pred_fake = D(fake_batch)
        loss_g = (torch.mean(torch.nn.ReLU()(1.0 + (y_pred - torch.mean(y_pred_fake)))) + torch.mean(
    torch.nn.ReLU()(1.0 - (y_pred_fake - torch.mean(y_pred))))) / 2
        loss_g.backward()
        optim_g.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--loss', type=str, default='relativgan', help='gan | wgan | relation | relativgan | ls')
    ...
    elif args.loss == 'relation':
        D = RE_Critic(args.nhidden, args.dnlayers)
    else:
    ...
    if args.loss == 'gan':
        ...
        ls_GAN(D, G, data, noise, niter=args.niters + 1, batch_size=args.batch_size,
                optimizer=args.optim, lrg=args.lrg, lrd=args.lrd, center=args.center, LAMBDA=args.LAMBDA,
                alpha=args.alpha, device=args.device, prefix=prefix, args=args)
    elif args.loss == 'relativgan':
        print(args.loss)
        RelativGAN(D, G, data, noise, niter=args.niters + 1, ncritic=args.ncritic, batch_size=args.batch_size,
                optimizer=args.optim, lrg=args.lrg, lrd=args.lrd, center=args.center, LAMBDA=args.LAMBDA,
                alpha=args.alpha, device=args.device, prefix=prefix, args=args)
    elif args.loss == 'relation':
        print(args.loss)
        REGAN(D, G, data, noise, niter=args.niters + 1, ncritic=args.ncritic, batch_size=args.batch_size,
                optimizer=args.optim, lrg=args.lrg, lrd=args.lrd, center=args.center, LAMBDA=args.LAMBDA,
                alpha=args.alpha, device=args.device, prefix=prefix, args=args)
