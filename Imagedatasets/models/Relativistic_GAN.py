...
class Relativistic_GAN(BaseModel):

    def name(self):
        return 'Relativistic_GAN'
    ...
    def backward_D(self):
        ...
        self.D_real = self.netD(self.input_img)
        D_fake = self.netD(self.fake)

        self.loss_D = (torch.mean(torch.nn.ReLU()(1.0 - (self.D_real - torch.mean(D_fake)))) + torch.mean(
            torch.nn.ReLU()(1.0 + (D_fake - torch.mean(self.D_real))))) / 2
    ...
    def backward_G(self):
        ...
        self.fake = self.netG(self.input_noise)
        if not hasattr(self,'D_reak'):
            with torch.no_grad():
                self.D_real = self.netD(self.input_img)
        D_fake = self.netD(self.fake)
        self.D_real = self.D_real.detach()
        self.loss_G = (torch.mean(torch.nn.ReLU()(1.0 + (self.D_real - torch.mean(D_fake)))) + torch.mean(
            torch.nn.ReLU()(1.0 - (D_fake - torch.mean(self.D_real))))) / 2
